...
def add_to_cart(request, item_id):
    ...
    # Retrieve the redirect URL from the form to return the user to the same
    # page after adding
    redirect_url = request.POST.get('redirect_url')
    # Get the color of the product (if applicable) from the POST request
    colour = request.POST.get('product_colour', None)
    # Get the current cart from the session (or initialize it as an empty
    # dictionary if none exists)
    ...
    if colour:
        ...
        # Update the quantity if the product and color combination already
        # exists in the cart
        if colour in cart[item_id]['items_by_colour']:
            cart[item_id]['items_by_colour'][colour] += quantity
            messages.success(
                request, f'Updated colour {colour.upper()} {product.name}
                quantity to {cart[item_id]["items_by_colour"][colour]}')
        else:
            # Add the color variant if it does not exist in the cart
            cart[item_id]['items_by_colour'][colour] = quantity
            messages.success(
                request, f'Added colour {colour.upper()} {product.name}
                to your cart')
    else:
...
def adjust_cart(request, item_id):
    ...
    if colour:
        if item_id in cart and colour in cart[item_id]['items_by_colour']:
            if quantity > 0:
                # Update the quantity if the specified amount is greater than
                # zero
                cart[item_id]['items_by_colour'][colour] = quantity
                messages.success(
                    request, f'Updated colour {colour.upper()} {product.name}
                ...
                if not cart[item_id]['items_by_colour']:
                    cart.pop(item_id)
                messages.success(
                    request, f'Removed colour {colour.upper()} {product.name}
                    from your cart')
    else:
        if quantity > 0:
...
def remove_from_cart(request, item_id):
    ...
    try:
        ...
        if colour:
            ...
            # Remove the product if no color is specified (i.e., it has no
            # color variants)
            if item_id in cart:
                cart.pop(item_id)
                messages.success(
