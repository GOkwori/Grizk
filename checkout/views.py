...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(
                request, "An error occurred while processing your payment. Please try again later.")
            logger.error(f"Stripe Error: {e}")
            return redirect(reverse('view_cart'))
        except Exception as e:
            messages.error(
                request, "A serious error occurred. Please try again later.")
...
def checkout_success(request, order_reference):
    ...
    messages.success(
        request, f'Order successfully processed! Your order number is {order_reference}. A confirmation email will be sent to {order.email}.')
